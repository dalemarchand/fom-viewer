...
function renderNoteIcon(noteName) {
  if (!noteName) return '';
  const note = findNoteByName(noteName);
  if (!note) return `<span class="note-icon" title="Note not found: ${noteName}">⚠️</span>`;
  ...
  return `<span class="note-icon clickable-item" onclick="showDetail('${noteName.replace(/'/g, "\\'")}', 'notes', true)" title="${tooltip.replace(/"/g, '&quot;')}">📝</span>`;
}
